...
function CreateProduct() {
    ...
    const handleAddVariation = () => {
        ...
        if (colors.includes(color) && sizes.includes(size)) {
            const isMatch = data.some(el => el.color === color && el.size === size)
            if (isMatch) {
                toast.error("Duplicate data entry")
            }
            else {
                data.push(variation)
                setdimensions(data)
                            ...
                            {dimensions.map((el, index) => {
                                return (
                                    <tr>
                                        ...
                                        <td><button type="button" className="btn-close" aria-label="Close" onClick={() => handleRemoveVariation(index)}></button></td>
                                    </tr>
                                )
                            })}
                        </tbody>
    ...
    return (
        <div>
            ...
            <form name="product-form" onSubmit={(e) => handleOnSubmit(e)}>
                ...
                <div className="mb-3 row">
                    <div className="col-sm-6">
                        <Radio title="Gender" name="gender" onChange={handleDataChange} data={genderData} />
                    </div>
                    <div className="col-sm-6">
                        <Radio title="For Sale" name="isSale" onChange={handleDataChange} data={saleData} />
                    </div>
                </div>
